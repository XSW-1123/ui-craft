...
const SLOP_LINE_RULES = [
  ...
  {
    id: 'dopamine-gradient', w: 2,
    re: /(linear|radial|conic)-gradient\([^)]*(?:#|rgb|hsl|var\()[^)]*(?:#|rgb|hsl|var\()[^)]*(?:#|rgb|hsl|var\()/i,
    msg: 'three-or-more-color gradient (dopamine blob risk)',
    hint: 'A gradient with 3+ stops reads as filler for empty space. Use a two-stop gradient bound to brand, or real content.',
  },
  {
    id: 'ai-palette', w: 3,
...
const SLOP_FILE_RULES = [
  ...
  {
    id: 'section-sameness', w: 3,
    test: (t) => {
      const sections = t.match(/<section\b[\s\S]*?<\/section>/gi) || [];
      if (sections.length < 3) return false;
      const repeated = sections.filter((s) =>
        /<h2[\s>]/i.test(s) &&
        /(grid-cols-3|repeat\(\s*3\s*,|repeat\(3,|columns:\s*3)/i.test(s)).length;
      return repeated >= 3;
    },
    msg: 'three or more sections share the same h2 + 3-column grid skeleton',
    hint: 'Title-subtitle-grid repeated down the page is the template rhythm. Vary section shape: full-bleed, split, list, quote, table. See references/saas-landing.md rule 8.',
  },
  {
    id: 'unnamed-quote', w: 2,
    test: (t) => {
      const regions = (t.match(/<blockquote[\s\S]*?<\/blockquote>/gi) || [])
        .concat(t.match(/<figure[\s\S]*?<\/figure>/gi) || []);
      return regions.some((q) =>
        /<img\b/i.test(q) &&
        !/cite\s*=/i.test(q) &&
        !/(—\s|@|role\s*[:=]|,\s*(CEO|CTO|Founder|Head|Lead|VP|Director|PM|Engineer))/i.test(q));
    },
    msg: 'testimonial with an avatar but no name, role, or cite',
    hint: 'Anonymous social proof is filler. Name, role-tag, and link the quote; or give the number a unit + window + subject. See references/saas-landing.md rule 7.',
  },
];
...
function detectClusters(map) {
  ...
  // D — AI Startup SaaS (2026 Persuade attractor): near-black ground + the AI-era marker set.
  // Harder to catch than B because it hides inside a plausible product page. Requires the dark
  // ground AND an AI-palette gradient together with at least one more AI-era tell.
  if (lum(bg) < 0.02) {
    const slopHits = new Set(findings.filter((f) => f.sev === 'SLOP').map((f) => f.rule));
    const hasAIPalette = slopHits.has('ai-palette') || slopHits.has('dopamine-gradient');
    const hasMore = ['marketing-filler', 'hero-blob', 'three-equal-cards', 'glassmorphism']
      .some((r) => slopHits.has(r));
    if (hasAIPalette && hasMore) {
      add('SLOP', 'cluster-ai-startup', 'tokens', 0,
        'near-black ground + AI-era palette + generated tells — attractor cluster D',
        'Cluster D (AI Startup SaaS). The 2026 Persuade trap: dark + neon + "AI era" copy + reflex grid. Escape needs a Signature only this brief could produce and real product copy, not the incantation. See references/anti-slop.md.', 3);
    }
  }
}
