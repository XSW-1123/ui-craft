...
metadata:
  version: 2.1.0
  synthesized_from: 2343 SKILL.md files across 33 repositories, 141 high-signal UI skills read in full
---
...
5. **Default aesthetics are a failure mode, gated.** The bundled audit computes a **Slop Index**
   ...
   and overrides the band in that case. See `references/anti-slop.md`.
...
- Marketing, landing, portfolio, launch, "wow", "惊艳" -> `VARIANCE 4, MOTION 2, DENSITY 2`
...
- **Persuade** (landing, pricing): one dominant focal point per viewport, generous whitespace,
  motion allowed to be expressive, copy is the design.
...
inputs, the three attractor clusters, uniform spacing, and token sprawl.
...
- [ ] Not one of the three default clusters; Signature move is present and singular
